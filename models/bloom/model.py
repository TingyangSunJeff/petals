...
class DistributedBloomModel(FromPretrainedMixin, PTuneMixin, BloomModel):
    ...
    def forward(
        ...
        logger.info(
            f"Running {self.__class__.__name__} with {len(self.h)} transformer layers on the swarm"
        )
        ...
        logger.info(f"before RemoteSequential.forward: {t1-t0:.3f}s")

        # do inference here --> RemoteSequential.forward
        ...
        logger.info(f"after RemoteSequential.forward: {t2-t0:.3f}s")
        # Remove prefix
        if use_prompts:
        ...
        logger.info(f"Remove prefix: {t3-t2:.3f}s")
        # Add last hidden state
        hidden_states = self.ln_f(hidden_states)
        hidden_states = hidden_states.view(output_shape)
        t4 = time.perf_counter()
        logger.info(f"Add last hidden state: {t4-t3:.3f}s")
        ...
        logger.info(f"BaseModelOutputWithPastAndCrossAttentions: {t5-t4:.3f}s")
        return result
