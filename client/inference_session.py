...
def log_time(stage, start):
    logger.info(f"[Timing] {stage}: {(time.time() - start):.4f}s")
...
class _ServerInferenceSession:
    ...
    def step(
        ...
        # compute bytes per tensor
        sizes = [t.element_size() * t.nelement() for t in input_tensors]
        total_bytes = sum(sizes)
        logger.info(f"[Timing] compressing payload: {total_bytes/1024:.1f} KB "
                        f"across {len(input_tensors)} tensors, "
                        f"individual sizes KB = {[s/1024 for s in sizes]}")
        ...
        log_time("compression_before_expert", compression_time)
        ...
        log_time("server_session_step_total", start_ss_step)
...
class InferenceSession:
    ...
    """
    def __init__(self, sequence_manager: RemoteSequenceManager, max_length: int, request_id: int = 1, routing_path: Optional[List[int]] = None):
    ...
    def __enter__(self) -> "InferenceSession":
        ...
        # report_memory_usage("Entering InferenceSession")
        return self
    ...
    def step(
        ...
        logger.info(f"===================================={self.count + 1}th token inference:")
        # per server loop
        while block_idx < self.num_blocks:
            wait_start_time = None  # <-- Add this line
            for attempt_no in itertools.count():
                ...
                try:
                    if not self._server_sessions or attempt_no >= 1:
                        if attempt_no == 1:
                            wait_start_time = time.time()  # <-- Start timing on first retry
                        # start_update_seq = time.time()
                    ...
                    end_time = time.time()
                    esapsed_time = end_time - start_time
                    ...
                    if wait_start_time is not None:
                        ...
                        logger.info(f"Waited {wait_time:.3f}s before successful retry for {self.request_id}th request server {server_session.span.peer_id})")
                    server_idx += 1
        ...
        total_time_one_token = 0
        for v, k in self.timer.items():
            logger.info("server:{}, {}th token average time:{} seconds".format(v, self.count + 1, np.mean(k)))
    ...
    def _update_sequence(self, server_idx: int, block_idx: int, attempt_no: int) -> int:
        ...
        logger.info(f"block_idx, update_end:{block_idx} {update_end}")
        ...
        logger.info(f"Found path from block {block_idx} to {update_end} via {len(updated_spans)} servers")
        ...
        # logger.info(f"***************{self._server_sessions.span}")
