...
class RemoteSequenceManager:
    ...
    def make_sequence(
        ...
        *,
        ...
        # logger.info(f"------> {self.state.sequence_info.spans_containing_block}")
    ...
    def _make_sequence_with_min_latency(
        ...
        for peer_id, block_idx in path.nodes[1:-1]:
            # logger.info(f"Path info: {peer_id} {block_idx}")
            ...
            # logger.info(f"span_sequence info: {span_sequence}")
    ...
    def _build_inference_graph(
        ...
        *,
        ...
        logger.info(f"client_server_rtts: {client_server_rtts}")
        graph = dijkstar.Graph()

        # Client -> server network delays
        for span in self.state.sequence_info.spans_containing_block[start_index]:
            logger.info(f"Client -> Server {client_server_rtts.get(span.peer_id)}")
            delay = self._rtt_to_delay(client_server_rtts.get(span.peer_id))
            logger.info(f"Client -> Server {span.peer_id}: Initial RTT delay = {delay:.6f} seconds")
            delay += overhead_delay
            logger.info(f"Client -> Server {span.peer_id}: Delay after adding overhead = {delay:.6f} seconds")
            if not self._has_cache_for(span, cache_tokens_needed):
                delay += alloc_delay
                logger.info(f"Client -> Server {span.peer_id}: Delay after adding allocation delay = {delay:.6f} seconds")
        ...
        for span in self.state.sequence_info.spans_containing_block[end_index - 1]:
            delay = self._rtt_to_delay(client_server_rtts.get(span.peer_id))
            logger.info(f"Server {span.peer_id} -> Client: Initial RTT delay = {delay:.6f} seconds")
        ...
        for block_idx in range(start_index + 1, end_index):
            for cur_span in self.state.sequence_info.spans_containing_block[block_idx - 1]:
                # logger.info(f"===>cur_span.server_info.next_pings: {cur_span.server_info.next_pings}")
                if cur_span.end != block_idx:
                    continue
                # logger.info(f"=======>{block_idx} {self.state.sequence_info.spans_containing_block[block_idx]}")
                for next_span in self.state.sequence_info.spans_containing_block[block_idx]:
                    ...
                    logger.info(f"Server {cur_span.peer_id} -> Server {next_span.peer_id}: Initial RTT delay = {delay:.6f} seconds")
                    delay += overhead_delay
                    # logger.info(f"Server {cur_span.peer_id} -> Server {next_span.peer_id}: Delay after adding overhead = {delay:.6f} seconds")
                    if not self._has_cache_for(next_span, cache_tokens_needed):
                        delay += alloc_delay
                        # logger.info(f"Server {cur_span.peer_id} -> Server {next_span.peer_id}: Delay after adding allocation delay = {delay:.6f} seconds")
                    graph.add_edge((cur_span.peer_id, block_idx), (next_span.peer_id, block_idx), delay)
                    logger.info(f"Added edge from {cur_span} to {next_span.peer_id} for block {block_idx} with delay {delay:.6f} seconds")

        # Compute delays
        for span in self.state.sequence_info.spans_by_priority:
            # logger.info(f"Server {span.peer_id}: Inference delay for block {block_idx} = {delay:.6f} seconds")
            ...
            for block_idx in range(max(span.start, start_index), min(span.end, end_index)):
                ...
                delay = 1.0 / inference_rps
                # logger.info(f"Server {span.peer_id}: Inference delay for block {block_idx} = {delay:.6f} seconds")
                graph.add_edge((span.peer_id, block_idx), (span.peer_id, block_idx + 1), delay)

        # # Log the graph details
        # logger.info(f"Graph nodes and edges:")
        # for node in graph.get_data():
        #     logger.info(f"Node: {node}")
        #     logger.info(f"dic: {graph.get_node(node)}")
        #     for edge in graph.get_node(node).get('edges', {}).items():
    ...
    def _update(self):
        ...
        with self.lock_changes:
            ...
            # logger.info(f"####################actual client fist server{first_servers}")
            middle_servers = [
        ...
        # logger.info(f"####################actual client first server{pinged_servers}")

        pinged_servers |= set(sample_up_to(middle_servers, self.config.max_pinged))
        # logger.info(f"####################actual client second server{middle_servers}")
        # logger.info(f"####################actual client middle server{pinged_servers}")

        pinged_servers |= set(sample_up_to(last_servers, self.config.max_pinged))
        # logger.info(f"####################actual client last server{pinged_servers}")
