...
class RemoteSequential(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, prompts: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        ...
        if self.active_session is None:
            ...
            result = _RemoteSequentialAutogradFunction.apply(inputs, prompts, self.sequence_manager)
            end_time = time.time()
            logger.info(f"RemoteSequential.forward (stateless) time: {end_time - start_time:.3f}s")
            return result
        else:
            #   --> InferenceSession.step()
            result = self.active_session.step(inputs, prompts, **kwargs)
            end_time = time.time()
            logger.info(f"RemoteSequential.forward (session) time: {end_time - start_time:.3f}s")
            return result
