...
def report_memory_usage(stage):
    ...
    print(f"[{stage}] Memory allocated for model: {allocated_memory / (1024**2):.2f} MB")
    print(f"[{stage}] Total reserved memory: {reserved_memory / (1024**2):.2f} MB")
...
def convert_block(
    ...
    **kwargs,
    ...
    # report_memory_usage("After quantization")
    # print(block.module_shards)
    ...
    for n, module in model.named_children():
        ...
        if isinstance(module, torch.nn.Linear) and n not in ["lm_head", "score"]:
            ...
            if quant_type == QuantType.INT8:
                ...
                new_linear_layer = bnb.nn.LinearNF4(
                    module.in_features,
                    module.out_features,
                    module.bias is not None,
                    compress_statistics=compress_statistics,
                )
                new_linear_layer.weight = bnb.nn.Params4bit(
                    module.weight.data,
                    requires_grad=False,
                    quant_type="nf4",
                    blocksize=64,
                ...
                new_linear_layer.bias = module.bias
                model._modules[n] = new_linear_layer
    return model
