...
class TransformerConnectionHandler(ConnectionHandler):
    ...
    async def rpc_inference(
        ...
        async with timeout(self.session_timeout):
            try:
                # here to wait request from client
            ...
            try:
                # the server parses the metadata and other details
        ...
        try:
            with self._managed_session(session_id) if session_id is not None else contextlib.nullcontext():
                while request.tensors:  # iterate while user is willing to supply tensors
                    # report_memory("Before inference step") # 10 times
            ...
            raise
        # finally:
        #     report_memory("After inference step")
