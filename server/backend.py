...
class TransformerBackend(ModuleBackend):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for descr in self.get_inference_cache_descriptors(batch_size=1, max_length=1):
            ...
            # logger.info("======> get_size_in_bytes(descr.dtype):{}".format(get_size_in_bytes(descr.dtype)))
            # logger.info("======> self.cache_bytes_per_token:{}".format(self.cache_bytes_per_token))
    ...
    def inference_step(
        ...
        def report_memory(stage: str):
            ...
            logger.info(f"[{stage}] Memory allocated: {allocated_memory / (1024**2):.2f} MB")
            logger.info(f"[{stage}] Total reserved memory: {reserved_memory / (1024**2):.2f} MB")
        ...
        # report_memory("before inference")
        # =======> actual tensor memory allocation
        with self.memory_cache.use_cache(
            *inference_info.cache_handles
            ...
            # reserved in `Server.()`. This saves us from OOMs if `max_chunk_size_bytes`
            ...
            for offset in range(0, seq_len, max_chunk_length):
                ...
                # report_memory(f"after extracting hidden_states_chunk at offset {offset}")
                # =====> 9MB diff
                # logger.info(f"{self.module}")
                ...
                logger.info(f"+++++++++++>>>>>inference time : {esapsed_time}")
                # report_memory(f"after forward pass at offset {offset}")
    ...
    def _update_cache_inplace(
        ...
        # logger.info("======> new_length:{}".format(new_length))
        for cache_key, new_key in zip(cache_tensors[0::2], new_kvs[0::2]):
            ...
            # logger.info("======> cache_key.shape:{}".format(cache_key.shape))
        for cache_value, new_value in zip(cache_tensors[1::2], new_kvs[1::2]):
            ...
            # logger.info("======> cache_value.shape:{}".format(cache_value.shape))
    ...
    def __call__(
        ...
        *optional_prompts: Optional[torch.Tensor],
        ...
        for inference_info, optional_prompt in zip(inference_infos, optional_prompts):
            ...
            logger.info("*****************_MergedInferenceStep")
