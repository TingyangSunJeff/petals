...
def report_memory_usage(stage):
    ...
    print(f"[{stage}] Memory allocated for model: {allocated_memory / (1024**2):.2f} MiB")
    print(f"[{stage}] Total reserved memory: {reserved_memory / (1024**2):.2f} MiB")

def print_block_parameters(block: nn.Module):
    total = 0
    for name, module in block.named_modules():
        ...
        for param_name, param in module.named_parameters(recurse=False):
            ...
            print(f"  Parameter: {param_name}, Number of parameters: {num_params}")
        total += total_params
        print(f"  Total parameters in {name}: {total_params}")
    print(f"  Total : {total}")

def get_quantized_size(tensor, quant_type):
    if quant_type == QuantType.NF4:
        # Each NF4 value uses 4 bits (0.5 bytes)
        return tensor.numel() * 0.5
    elif quant_type == QuantType.INT8:
        # Each INT8 value uses 1 byte
        return tensor.numel() * 1
    else:
        return tensor.numel() * get_size_in_bytes(tensor.dtype)
            
def compute_block_size(block: nn.Module, quant_type: QuantType):
    num_parameters = 0
    block_size = 0

    for name, submodule in block.named_modules():
        for param_name, param in submodule.named_parameters(recurse=False):
            param_size = param.numel() * get_size_in_bytes(param.dtype)
            num_parameters += param.numel()
            block_size += param_size

            logger.info(f"Parameter: {name}.{param_name}, Shape: {param.shape}, dtype: {param.dtype}, Size: {param_size / (1024**2):.2f} MB")

    logger.info(f"Total number of parameters: {num_parameters}")
    logger.info(f"Total block size: {block_size / (1024**2):.2f} MB")
...
class Server:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        tseice  = get_size_in_bytes(self.torch_dtype)
        ...
        logger.info(f"Attention cache for all blocks will consume up to {self.attn_cache_bytes / gb:.2f} GB")
        ...
        logger.info(f"throughput_info{throughput_info}")
        ...
        # # File and lock setup
        # mat_file = 'module_infos.mat'
        # lock = FileLock(f"{mat_file}.lock")

        # # Attempt to acquire lock and update file
        # with lock:
        #     try:
        #         # Load existing data (if file exists)
        #         try:
        #             existing_data = loadmat(mat_file)
        #         except FileNotFoundError:
        #             existing_data = {}

        #         # Extract the throughput and server ID
        #         throughput_value = throughput_info["throughput"]
        #         server_id = visible_maddrs_str[0].split("/")[-1]

        #         # Update or add new entry
        #         existing_data[server_id] = throughput_value

        #         # Save the updated data back to the .mat file
        #         savemat(mat_file, existing_data)
        #         logger.info(f"Adding pair {server_id}: {throughput_value} to mat")
        #     except Exception as e:  
        ...
        # logger.info(f"Model weights are loaded in {self.strict_block_indices} format")
    ...
    def _choose_num_blocks(self) -> int:
        ...
        logger.info(f"Total memory available for the server: {total_memory / (10**9):.2f} GB")
        gib = 1024**3
        ...
        # logger.info(f"========>{self._cache_bytes_per_block} {block_size} {self.quant_type}")
        if self.adapters:
        ...
        # logger.info(f"========>{total_memory} {autograd_memory} {total_memory_per_block}")
        num_blocks = min(num_blocks, self.block_config.num_hidden_layers)
        logger.info(
...
class ModuleContainer(threading.Thread):
    ...
    def create(
        ...
        **kwargs,
        ...
        dht_announcer.start()
        # logger.info(f"next ping info: {dht_announcer.ping_aggregator.to_dict()}")
        ...
        try:
            for module_uid, block_index in zip(module_uids, block_indices):
                ...
                # print_block_parameters(block)
                # report_memory_usage("After convert_block")
                # compute_block_size(block, quant_type)
                ...
                report_memory_usage(f"Block {block_index} loaded")
    ...
    def run(self) -> None:
        while True:
            ...
            if self.server_info.state != ServerState.OFFLINE:
                ...
                logger.info(f"$$$$$$$$$$$$ ping next happens{self.server_info.next_pings}")
            else:
