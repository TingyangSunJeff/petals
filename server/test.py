...
#         logger.info(f"cache_dir: {DEFAULT_CACHE_DIR}")
...
#             throughput_info = measure_throughput_info(
...
#             throughput_forward = throughput_info["forward_rps"] / average_blocks_used
#             throughput = throughput_forward
#             throughput_inference = throughput_info["inference_rps"] / average_blocks_used
#             inference_time = 1/float(throughput_info["inference_rps"])
#             network_rps = throughput_info["network_rps"] * (relay_penalty if reachable_via_relay else 1)
            
#             network_time = 1/float(network_rps)
#             throughput = min(throughput, network_rps)
            
#             throughput_info["throughput"] = throughput
#             # logger.info("Reporting average_blocks_used: {}".format(average_blocks_used))
#             # logger.info(f"Reporting throughput: {throughput:.1f} tokens/sec for {num_blocks} blocks")
#             # logger.info("Reporting average_throughput_info['network_rps']: {}".format(throughput_info["network_rps"]))
#             # logger.info(f"Reporting forward throughput: {throughput_forward:.1f} tokens/sec for {num_blocks} blocks")
#             # logger.info(f"Reporting inference throughput: {throughput_inference:.1f} tokens/sec for {num_blocks} blocks")
#             # logger.info("Reporting inference time: {} sec".format(inference_time))
#             # logger.info("Reporting network transmission time for per request: {} sec".format(network_time))
#             # logger.info("Reporting network transmission + inference time: {} sec".format(inference_time + network_time))

#             cache[cache_key] = throughput_info
...
#     full_throughput_dict = cache[cache_key]
#     return full_throughput_dict
...
#                 print(f"Server ID: {server['id']}, Name: {server['name']}, Location: {server['country']}")

#         # Get the best server based on ping time and print its details
#         best_server = s.get_best_server()
#         print(f"Best Server ID: {best_server['id']}, Name: {best_server['name']}, Location: {best_server['country']}")
#         s.get_best_server()
#         s.download()
#         s.upload()
