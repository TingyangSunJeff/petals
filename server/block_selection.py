...
def _choose_best_start(throughputs: np.ndarray, num_blocks: int) -> int:
    ...
    # options1 = [(sorted(throughputs[i : i + num_blocks]), i) for i in range(0, len(throughputs) - num_blocks + 1)]
    # logger.info(f"********-----> {options1}")
    # it will always choose the tuple with smallest first element
